...
class DBHelper:
    ...
    def add_user_with_language(self, userID, lang):
        stmt = "INSERT INTO users (userID, lang, interactionCount, reminderCount, readsuraCount, stage, praytimecount) SELECT %s, %s, %s, %s, %s, %s, %s WHERE NOT EXISTS (SELECT userID FROM users WHERE userID::int = %s);"
        args = (userID, lang, 0, 0, 0, 0, 0, userID,)
        self.cur.execute(stmt, args)
    ...
    def update_user_praytimecount(self, userID):
        stmt = "UPDATE users SET praytimecount = praytimecount + 1 WHERE userID::int = %s;"
        args = (userID,)
        self.cur.execute(stmt, args)
    ...
    def get_message_toSend(self):
        stmt = "SELECT messagetext FROM messages WHERE messageid = 1;"
        self.cur.execute(stmt)
        row = str(self.cur.fetchone())[2:-3]
        return row

    def get_users(self):
        stmt = "SELECT userid FROM users;"
        self.cur.execute(stmt)
        row = self.cur.fetchall()
        return row

    def delete_specialmessage(self):
        stmt = "DELETE FROM messages WHERE messageid = 1;"
        self.cur.execute(stmt)

    def get_sumof_dailyTraffic(self):
        stmt = "SELECT SUM(interactioncount) AS total FROM users;"
        self.cur.execute(stmt)
        row = str(self.cur.fetchall()[0])[1:-2]
        return row

    def get_sumof_dailySurah(self):
        stmt = "SELECT SUM(readsuracount) AS total FROM users;"
        self.cur.execute(stmt)
        row = str(self.cur.fetchall()[0])[1:-2]
        return row

    def get_sumof_dailySPray(self):
        stmt = "SELECT SUM(praytimecount) AS total FROM users;"
        self.cur.execute(stmt)
        row = str(self.cur.fetchall()[0])[1:-2]
        return row

    def get_sumof_nlocCount(self):
        stmt = "SELECT SUM(praytimecount) AS total FROM (select praytimecount from users where cityname is null and locationlong is not null) AS praytimecount;"
        self.cur.execute(stmt)
        row = str(self.cur.fetchall()[0])[1:-2]
        return row

    def insert_dailyReport(self, reportDate):
        stmt = "INSERT INTO trafficdb (trafficdate, trafficcount, totalreadsura, totalpraytime, totalpraytimenotloc) VALUES (%s, %s, %s, %s, %s);"
        args = (reportDate, self.get_sumof_dailyTraffic(), self.get_sumof_dailySurah(), self.get_sumof_dailySPray(), self.get_sumof_nlocCount(), )
        self.cur.execute(stmt, args)

    def update_usersAfterReport(self):
        stmt = "UPDATE users SET interactioncount = 0, remindercount = 0, readsuracount = 0, praytimecount = 0;"
        self.cur.execute(stmt)
